...
| 4 | El manejador de PWM nativo del micro:bit no es independiente por pin — motores y salida analógica lo comparten, reportado en `microsoft/pxt-microbit#4950`, sin confirmar si difiere entre V1 y V2. | Medio — condiciona el diseño de Tarea 5 y Tarea 9 | Tarea 5 (motores) ya cerrada con código; T4 de `test.ts` y el procedimiento de V4 en `VERIFICACION.md` proveen la verificación cruzada con servos y salida analógica. **Pendiente ejecutar con la placa real** — relacionado con la consulta 6 (¿la placa fue validada con V1, V2, o ambas?), también a anotar en V4. |
...
| 9 | Verificando ANA-1 a ANA-4 con placa (potenciómetro en 3,3V/GND, monitor serie), los puertos 1–3 recorrían 0 a 100 pero los puertos 4–6 topeaban en 81, violando ANA-2 (~19 puntos de diferencia entre torres). No era hardware ni bug de lógica: consecuencia matemática de dividir por el fondo de escala *configurado* del ADS1015 (2047 cuentas = 4,096 V, D3) en vez del techo eléctrico *real* de la señal (3,3 V) — con 2 mV/cuenta, 3,3 V son 1650 cuentas, y `round(1650*100/2047)=81`, exactamente el valor medido. Detectado por Santi el 2026-08-29 verificando V2. | Alto — violaba ANA-2 directamente en los puertos 4, 5 y 6 | **Resuelto** — `analog.ts` recalibrado para dividir por 1650 (la cuenta real a 3,3 V) en vez de 2047, igualando el significado de "100" al mismo techo físico en los seis puertos. Fundamento completo en `ARQUITECTURA.md` §8 (D1). Confirmado con la placa: las tres torres llegan a 100 (`VERIFICACION.md` V2, cumple). |
| 10 | El README (Tarea 7) tiene que decirle al docente que los motores no frenan activamente — se detienen y siguen girando por inercia — y el límite de corriente de motores (~1,2 A continuos por canal, `ARQUITECTURA.md` §3.7). Detectado al cerrar la Tarea 5: ningún criterio de aceptación actual de DOC-1 ni DOC-2 lo exige — DOC-2 solo habla explícitamente del límite de corriente de servos. | Medio — un docente sin este dato puede sorprenderse en el aula, o esperar que el motor frene igual que se detiene un servo | Abierto, para la Tarea 7. |
| 11 | El desplegable de `MotorDirection` aparecía en inglés ("forward"/"backward") con el editor en español, pese a tener entradas en `_locales/es-ES/pxt-kroma-strings.json`. Detectado por Santi probando la Tarea 5. Causa: la clave de locale de un enumerado declarado dentro de un `namespace` lleva el prefijo del namespace (`namespace.EnumName.Member\|block`), no solo `EnumName.Member\|block` — confirmado contra una extensión real con traducciones de Crowdin (`pxt-common-packages/libs/game`, enum `AnimationTypes` dentro de `namespace animation`, clave real `animation.AnimationTypes.All\|block`). El mismo error afectaba en silencio a `KromaCompareOp` desde la Tarea 10, invisible porque sus rótulos ("=", "<", ">") son iguales en los dos idiomas. | Medio — motores se notó de inmediato; `KromaCompareOp` llevaba roto desde la Tarea 10 sin que nadie lo notara | **Resuelto** — las cinco claves de enumerado en `_locales/es-ES/pxt-kroma-strings.json` (`KromaCompareOp` ×3, `MotorDirection` ×2) llevan ahora el prefijo `kroma.`. Pendiente confirmar visualmente con el editor en español, no solo leyendo el JSON. |
