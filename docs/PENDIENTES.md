...
| 8 | Puertos 3 y 5 (pines 0 y 3 del PCA9536 — los dos bits extremos del nibble de `configMirror`/`outputMirror`) no responden a `digitalOutput`: quedan fijos en ~3,3 V. Primera vez que se prueba DIG-1/2/3 físicamente sobre los seis puertos (Santi, 2026-08-29). Diagnóstico con serial descartable descartó las dos sospechas iniciales con datos reales de placa: el lookup `findPortEntry` resuelve el pin correcto para ambos puertos, y `configMirror`/`outputMirror` siguen exactamente la secuencia de bits esperada. Detalle completo en `VERIFICACION.md` V1. | Alto — bloquea el cierre físico de DIG-1/2/3 (Tarea 2) para dos de los seis puertos | **Abierto.** Candidatos sin confirmar: problema físico en los pines extremos del PCA9536, o falla de escritura I2C no aislable todavía porque la lectura de registros (CONFIG/OUTPUT) usada para depurar es instrumentación nueva, nunca antes probada en placa real, y da resultados (siempre 0) que podrían ser un artefacto propio. Siguiente paso: medir con multímetro directo sobre los pines del chip. |
