...
- **Disparar evento de puerto:** `board.ts` (`onDigitalPortEvent`/`onAnalogCompareEvent`) registra el watch en `events.ts` (`watchDigital`/`watchAnalog`, con deduplicación por puerto+condición) y se suscribe con `control.onEvent` a la fuente que devuelve `control.allocateEventSource()` — asignada dinámicamente y libre de colisión por construcción, en vez de un rango de constantes elegido a mano (confirmado contra el código fuente real de `pxt-microbit` 9.0.12 al implementar). Un único fiber en segundo plano (`events.ts`, inicialización perezosa con el mismo patrón `initialized`/`ensureInitialized()` de §6.2) sondea cada `POLL_INTERVAL_MS` los puertos con algún watch registrado, cacheando una lectura por puerto (no una por watch) para no repetir tráfico I2C entre watches del mismo puerto. Cada watch exige varios sondeos consecutivos con la misma condición antes de confirmar y disparar (antirrebote) — incluido el primer sondeo: si la condición ya está cumplida en el momento de registrar el evento, se confirma y dispara igual, sin esperar una transición real. Al confirmarse la condición (ya sea de entrada o por una transición posterior), dispara `control.raiseEvent` con la fuente y el valor exactos con los que el bloque se registró — mismo valor para dos bloques que compartan puerto y condición, así ambos manejadores se ejecutan sin lógica propia.
...
**D-MOT-a — Período de PWM de motores: 40 µs (25 kHz), con escalera de repliegue (Tarea 5).** `HARDWARE.md` §9.7 sugiere bajar el período a "unos pocos cientos de microsegundos" — un criterio eléctrico: alcanza con que el período sea mucho menor que la constante de tiempo L/R del motor para que la corriente no colapse entre ciclos. Es un criterio correcto pero no alcanza para MOT-6, que es acústico y más exigente: unos pocos kHz (el rango que da un período de unos pocos cientos de µs) cae justo en la banda más sensible del oído humano, así que corregiría el giro a tirones sin corregir el chillido. Se implementa contra la especificación, que manda en la cadena de autoridad — la sugerencia de `HARDWARE.md` no se corrige (es relevamiento del proveedor), pero la discrepancia queda registrada acá. Valor elegido: 40 µs (25 kHz), arriba del rango audible incluso para un oído infantil, dentro del límite de conmutación del TB6612 (100 kHz). Escalera de repliegue fijada de antemano si V4 encuentra pérdida de torque, un arranque a velocidad notablemente más alta, o muy pocos escalones útiles entre "no arranca" y "al máximo": 40 µs → 50 µs (20 kHz) → 100 µs (10 kHz) → 250 µs (4 kHz, piso). Bajar de escalón acerca el ruido a la banda audible, así que no se hace por prolijidad. Si hace falta llegar al escalón 3 o 4, MOT-6 queda en riesgo y se reporta como hallazgo en `PENDIENTES.md`. Valor final y motivo, tras V4: pendiente de verificación con la placa (ver `VERIFICACION.md` V4).
